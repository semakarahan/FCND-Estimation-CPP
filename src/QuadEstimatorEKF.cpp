...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  //if (ekfState(6) < -F_PI) ekfState(6) += 2.f*F_PI;
    ...
    // Hint-1 is used for the solution.
    Mat3x3F R;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(3) += acc_I.x * dt;
  predictedState(4) += acc_I.y * dt;
  predictedState(5) += (acc_I.z - CONST_GRAVITY) * dt ;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  // Reference: Estimation for Quadrotors Equation 52
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // Reference: Estimation for Quadrotors Equation 51
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  // Reference: Estimation for Quadrotors Equation 53 & 55
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  float yaw_error = magYaw - zFromX(0);
